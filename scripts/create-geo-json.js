...
const writeGeoJsonForServices = (currentServiceList, outputName) => {
  ...
  const ICBs = safeRequire("../archive/ICBs-2023.json", {
    type: "FeatureCollection",
    features: [],
  });

  const NorthernIreland = safeRequire("../archive/northern-ireland.json", {
    type: "FeatureCollection",
    features: [],
  });
  const AllWales = safeRequire("../archive/all-wales.json", {
    type: "FeatureCollection",
    features: [],
  });

  const Counties = safeRequire(
    "../archive/Counties_(December_2020)_EN_BGC.json",
    { type: "FeatureCollection", features: [] }
  );

  const CcgEngland21 = safeRequire(
    "../archive/Clinical_Commissioning_Groups_(April_2021)_EN_BUC.json",
    { type: "FeatureCollection", features: [] }
  );
  const CcgEngland20 = safeRequire(
    "../archive/Clinical_Commissioning_Groups_(April_2020)_EN_BFC_V2.json",
    { type: "FeatureCollection", features: [] }
  );
  const CcgEngland19 = safeRequire(
    "../archive/Clinical_Commissioning_Groups__April_2019__Boundaries_EN_BUC.json",
    { type: "FeatureCollection", features: [] }
  );
  const CcgEngland17 = safeRequire(
    "../archive/Clinical_Commissioning_Groups_(April_2017)_Boundaries_(Version_4).json",
    { type: "FeatureCollection", features: [] }
  );
  const CcgEngland16 = safeRequire(
    "../archive/Clinical_Commissioning_Groups_(April_2016)_Boundaries.json",
    { type: "FeatureCollection", features: [] }
  );
  const CcgEngland15 = safeRequire(
    "../archive/Clinical_Commissioning_Groups_(July_2015)_Boundaries.json",
    { type: "FeatureCollection", features: [] }
  );

  const PostcodeCH = safeRequire("../archive/welsh-postcodes/CH.json", {
    type: "FeatureCollection",
    features: [],
  });
  const PostcodeLD = safeRequire("../archive/welsh-postcodes/LD.json", {
    type: "FeatureCollection",
    features: [],
  });
  const PostcodeLL = safeRequire("../archive/welsh-postcodes/LL.json", {
    type: "FeatureCollection",
    features: [],
  });
  const PostcodeSY = safeRequire("../archive/welsh-postcodes/SY.json", {
    type: "FeatureCollection",
    features: [],
  });
  const PostcodeCF = safeRequire("../archive/welsh-postcodes/CF.json", {
    type: "FeatureCollection",
    features: [],
  });
  const PostcodeHR = safeRequire("../archive/welsh-postcodes/HR.json", {
    type: "FeatureCollection",
    features: [],
  });
  const PostcodeSA = safeRequire("../archive/welsh-postcodes/SA.json", {
    type: "FeatureCollection",
    features: [],
  });
  const PostcodeNP = safeRequire("../archive/welsh-postcodes/NP.json", {
    type: "FeatureCollection",
    features: [],
  });

  const Scotland = safeRequire("../archive/health-boards-small.json", {
    type: "FeatureCollection",
    features: [],
  });

  const LondonBoroughs = safeRequire(
    "../archive/Upper_Tier_Local_Authorities_December_2022_Boundaries_UK_BFC_-582766102348601148.json",
    { type: "FeatureCollection", features: [] }
  );
  ...
  for (const currentService of currentServiceList) {
    ...
    for (const currentCcg of uniqueCodes) {
      ...
      for (const currentSource of PRIORITY_ORDER_GEOJSON) {
        if (!featureForCcg) {
          ...
          if (
            ...
            (!currentSource.map.features ||
              currentSource.map.features.length === 0)
          ) {
      ...
      const isWalesPlaceholder =
        currentCcg.toLowerCase() === "w99999999" ||
        currentCcg.toLowerCase() === "all-wales";

      // Fallback: If it's a Welsh postcode or Wales placeholder code, use all-wales.json
      if (
        !featureForCcg &&
        (isWelshPostcode || isWalesPlaceholder) &&
        AllWales.features &&
        AllWales.features.length > 0
      ) {
